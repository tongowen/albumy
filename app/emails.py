...
def send_mail(to, subject, template, **kwargs):
    message = Message(current_app.config['O_MAIL_SUBJECT_PREFIX'] + subject, recipients=[to])
...
def send_change_email_email(user, token, to=None):
    send_mail(subject='更换邮箱', to=to or user.email, template='emails/change_email', user=user, token=token)
